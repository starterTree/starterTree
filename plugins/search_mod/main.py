...
def fill_tableau(n, tableau_keys, path_entry_name_content_entry, tableau):
    ...
    for i in tableau_keys:
        if i in path_entry_name_content_entry:
            if (i == "content" or i == "description") and "type" in path_entry_name_content_entry and \
                    path_entry_name_content_entry["type"] == "www":
                tableau_lines.append(
                    "[link=" + path_entry_name_content_entry[i] + "]" + path_entry_name_content_entry[i] + "[/link]")
            else:
                    ...
                    if i.split("=")[0] in "tag,name,type,path":
                        if len(i.split("=")) == 2:
                            ...
                            if i.split("=")[0] == "tag":
                                ...
                                for t in i.split("=")[1].split(","):
                                    if t is not "":
                                        if 'tags' in path_entry_name_content[r] and (
                                                t in path_entry_name_content[r]["tags"]):
                            ...
                            if i.split("=")[0] == "name":
                                queryT = queryT + str(bool(
                                    re.search(i.split("=")[1], path_entry_name_content[r]["name"],
                                              re.IGNORECASE))) + " "
                            if i.split("=")[0] == "path":
                                queryT = queryT + str(bool(
                                    re.search(i.split("=")[1], path_entry_name_content[r]["path"],
                                              re.IGNORECASE))) + " "
                            if i.split("=")[0] == "type":
                                if 'type' in path_entry_name_content[r] and (
                                        i.split("=")[1].lower() == path_entry_name_content[r]["type"] or i.split("=")[
                                    1].upper() == path_entry_name_content[r]["type"]):
...
plugin = Plugin(namePlugin="search", dataYaml=dataYaml, runInMenu=runInMenu, icon="⠀", titleIcon="",
                options=["debug"])
